...
namespace CodeGamified.Engine
{
    ...
    public readonly struct Instruction
    {
        ...
        public string ToAssembly()
        {
            return Op switch
            {
                OpCode.LOAD_CONST => $"LDI R{Arg0}, {Arg1}",
                OpCode.LOAD_FLOAT => $"LDF R{Arg0}, #{Arg1}",
                OpCode.LOAD_MEM   => $"LDM R{Arg0}, [{Arg1}]",
                OpCode.STORE_MEM  => $"STM [{Arg1}], R{Arg0}",
                ...
                OpCode.MIN  => $"MIN R{Arg0}, R{Arg1}",
                OpCode.MAX  => $"MAX R{Arg0}, R{Arg1}",
                OpCode.INC  => $"INC R{Arg0}",
                OpCode.DEC  => $"DEC R{Arg0}",
                OpCode.CMP  => $"CMP R{Arg0}, R{Arg1}",
                OpCode.JMP  => $"JMP @{Arg0:D4}",
                OpCode.JEQ  => $"JEQ @{Arg0:D4}",
                OpCode.JNE  => $"JNE @{Arg0:D4}",
                OpCode.JLT  => $"JLT @{Arg0:D4}",
                OpCode.JGT  => $"JGT @{Arg0:D4}",
                OpCode.JLE  => $"JLE @{Arg0:D4}",
                OpCode.JGE  => $"JGE @{Arg0:D4}",
                OpCode.PUSH => $"PUSH R{Arg0}",
                OpCode.POP  => $"POP R{Arg0}",
                OpCode.CALL => $"CALL @{Arg0:D4}",
                OpCode.RET  => "RET",
                OpCode.WAIT => $"WAIT R{Arg0}",
                OpCode.NOP  => "NOP",
                OpCode.HALT => "HLT",
                OpCode.BREAK => "BRK",
                _ => $"IO.{(int)Op - 100,2} {Arg0}, {Arg1}, {Arg2}"
            };
        }

        /// <summary>Format with a game-specific custom opcode formatter.</summary>
        public string ToAssembly(System.Func<Instruction, string> customFormatter)
        {
            if (customFormatter != null && Op >= OpCode.CUSTOM_0)
